				 ...
				 struct jffs2_raw_inode *ri, uint32_t ofs, struct jffs2_summary *s)
{
	...
	{
	char * tmpbuf = kmalloc(je32_to_cpu(ri->dsize) * sizeof(char) + 1, GFP_KERNEL);
	memcpy(tmpbuf, ri->data, je32_to_cpu(ri->dsize));
	tmpbuf[je32_to_cpu(ri->dsize)] = '\0';
	printk("Pierre : data node #%u found (version %d), range 0x%x-0x%x, data :\n%s\n",
		je32_to_cpu(ri->ino), je32_to_cpu(ri->version), je32_to_cpu(ri->offset),
		  je32_to_cpu(ri->offset)+je32_to_cpu(ri->dsize), tmpbuf);
	kfree(tmpbuf);
	}
				  ...
				  struct jffs2_raw_dirent *rd, uint32_t ofs, struct jffs2_summary *s)
{
	...
	printk("Pierre : dentry node found for #%u, version=%d, name=%s\n",
						je32_to_cpu(rd->ino), je32_to_cpu(rd->version), fd->name);
